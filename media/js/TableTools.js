...
function TableTools ( oInit )
{
	...
	function fnFeaturePrint ()
	{
		...
		$(nButton).click( function() {
			//if print from accounts.php
			
			if(_oSettings.sPrintMessage==1)
			{
				poptastic("print.php?it=5");
			}
			else
			{
			...
			setTimeout( function() {
				...
				$('.dataTables_scrollBody').addClass('noScrollY');
				$('.gridTitle,.gridHide').css('display','none');
				$('.dataTables_scrollHead').css('display','table-header-group');
				$('.dataTables_scrollFoot').css('display','table-footer-group');
	...
	function fnPrintEnd ( e )
	{
		...
		if ( e.keyCode == 27 )
		{
			...
			$('.dataTables_scrollBody').removeClass('noScrollY');
			$('.gridTitle,.gridHide').css('display','table-cell');
		}
	}
	...
	function fnGetDataTablesData( sSeperator, sBoundary )
	{
		...
		sData = sData.replace(/(<strong>)|(<\/strong>)|(<h5>)|(<\/h5)|(<h6>)|(<\/h6>)|(<span>)|(<\/span>)/g,"");
		var sLoopData = '';
