...
//Authorization.
if(!empty($_GET['id'])){
	$auth->roleid="8943";//Edit
}
else{
	$auth->roleid="8941";//Add
}
...
if($obj->action=="Save"){
	...
	$error=$techunderresearch->validate($obj);
	if(!empty($error)){
		$error=$error;
	}
	else{
		//$techunderresearch=$techunderresearch->setObject($obj);
		if($techunderresearch->add($obj)){
			$error=SUCCESS;
			redirect("addtechunderresearch_proc.php?id=".$techunderresearch->id."&error=".$error);
		}
		else{
			$error=FAILURE;
		}
...
if(!empty($id)){
	...
	$obj=$techunderresearch->fetchObject;
...
include "addtechunderresearch.php";
?>