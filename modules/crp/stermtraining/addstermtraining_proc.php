...
//Authorization.
if(!empty($_GET['id'])){
	$auth->roleid="8919";//Edit
}
else{
	$auth->roleid="8917";//Add
}
...
if($obj->action=="Save"){
	...
	$error=$stermtraining->validate($obj);
	if(!empty($error)){
		$error=$error;
	}
	else{
		//$stermtraining=$stermtraining->setObject($obj);
		if($stermtraining->add($obj)){
			$error=SUCCESS;
			redirect("addstermtraining_proc.php?id=".$stermtraining->id."&error=".$error);
		}
		else{
			$error=FAILURE;
		}
...
if(!empty($id)){
	...
	$groupby="";
	$orderby="";
	$stermtraining->retrieve($fields,$join,$where,$having,$groupby,$orderby);
	$obj=$stermtraining->fetchObject;
