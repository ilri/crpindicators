...
//Authorization.
if(!empty($_GET['id'])){
	$auth->roleid="8935";//Edit
}
else{
	$auth->roleid="8933";//Add
}
...
if($obj->action=="Save"){
	...
	$error=$technpracticreleasd->validate($obj);
	if(!empty($error)){
		$error=$error;
	}
	else{
		//$technpracticreleasd=$technpracticreleasd->setObject($obj);
		if($technpracticreleasd->add($obj)){
			$error=SUCCESS;
			redirect("addtechnpracticreleasd_proc.php?id=".$technpracticreleasd->id."&error=".$error);
		}
		else{
			$error=FAILURE;
		}
...
if($obj->action=="Update"){
	...
	if(!empty($error)){
		...
		//$technpracticreleasd=$technpracticreleasd->setObject($obj);
		if($technpracticreleasd->edit($obj)){
			$error=UPDATESUCCESS;
			redirect("addtechnpracticreleasd_proc.php?id=".$technpracticreleasd->id."&error=".$error);
		}
		else{
...
if(!empty($id)){
	...
	$obj=$technpracticreleasd->fetchObject;
...
include "addtechnpracticreleasd.php";
?>