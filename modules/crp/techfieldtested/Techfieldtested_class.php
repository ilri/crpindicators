	...
	function edit($obj,$where=""){
		...
		if($techfieldtestedDBO->update($obj,$where)){
			...
			while($row=mysql_fetch_object($valuechains->result)){
			  ...
			  //$array[$row->id] = $aRow['valuechain'.$id];
			  if(!empty($aRow['valuechain'.$id])){
			    $sql="select * from indvlcalloc where tableid='$tableid' and valueid='$valueid' and valuechainid='$id'";echo $sql;
			    $rs = mysql_query($sql);
			    if(mysql_affected_rows()>0){
			      //do nothing
			      $r=mysql_fetch_object($rs);
			...
			$ids=substr($ids,0,-1);echo "delete from indvlcalloc where id not in($ids) and tableid=$tableid and valueid=$valueid";
