...
//Authorization.
if(!empty($_GET['id'])){
	$auth->roleid="8931";//Edit
}
else{
	$auth->roleid="8929";//Add
}
...
if($obj->action=="Save"){
	...
	$error=$techfieldtested->validate($obj);
	if(!empty($error)){
		$error=$error;
	}
	else{
		//$techfieldtested=$techfieldtested->setObject($obj);
		if($techfieldtested->add($obj)){
			$error=SUCCESS;
			redirect("addtechfieldtested_proc.php?id=".$techfieldtested->id."&error=".$error);
		}
		else{
			$error=FAILURE;
		}
...
if(!empty($id)){
	...
	$groupby="";
	$orderby="";
	$techfieldtested->retrieve($fields,$join,$where,$having,$groupby,$orderby);
	$obj=$techfieldtested->fetchObject;
...
include "addtechfieldtested.php";
?>