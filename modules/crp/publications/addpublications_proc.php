...
//Authorization.
if(!empty($_GET['id'])){
	$auth->roleid="8911";//Edit
}
else{
	$auth->roleid="8909";//Add
}
...
if($obj->action=="Save"){
	...
	$error=$publications->validate($obj);
	if(!empty($error)){
		$error=$error;
	}
	else{
		//$publications=$publications->setObject($obj);
		if($publications->add($obj)){
			$error=SUCCESS;
			redirect("addpublications_proc.php?id=".$publications->id."&error=".$error);
		}
		else{
			$error=FAILURE;
		}
