...
//Authorization.
if(!empty($_GET['id'])){
	$auth->roleid="8883";//Edit
}
else{
	$auth->roleid="8881";//Add
}
...
if($obj->action=="Save"){
	...
	$error=$flagshipprdcts->validate($obj);
	if(!empty($error)){
		$error=$error;
	}
	else{
		//$flagshipprdcts=$flagshipprdcts->setObject($obj);
		if($flagshipprdcts->add($obj)){
			$error=SUCCESS;
			redirect("addflagshipprdcts_proc.php?id=".$flagshipprdcts->id."&error=".$error);
		}
		else{
			$error=FAILURE;
		}
...
if(!empty($id)){
	...
	$obj=$flagshipprdcts->fetchObject;
