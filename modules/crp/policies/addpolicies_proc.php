...
//Authorization.
if(!empty($_GET['id'])){
	$auth->roleid="8907";//Edit
}
else{
	$auth->roleid="8905";//Add
}
...
if($obj->action=="Save"){
	...
	$error=$policies->validate($obj);
	if(!empty($error)){
		$error=$error;
	}
	else{
		//$policies=$policies->setObject($obj);
		if($policies->add($obj)){
			$error=SUCCESS;
			redirect("addpolicies_proc.php?id=".$policies->id."&error=".$error);
		}
		else{
			$error=FAILURE;
		}
