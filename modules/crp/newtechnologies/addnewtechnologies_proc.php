...
//Authorization.
if(!empty($_GET['id'])){
	$auth->roleid="8899";//Edit
}
else{
	$auth->roleid="8897";//Add
}
...
$id=$_GET['id'];
echo $_SESSION['newtechnologiesid'];
if(!empty($_SESSION['newtechnologiesid'])){
  $id=$_SESSION['newtechnologiesid'];
  $_SESSION['newtechnologiesid']="";
}
...
if($obj->action=="Save"){
	...
	$error=$newtechnologies->validate($obj);
	if(!empty($error)){
		$error=$error;
		return $error;
	}
	else{
		//$newtechnologies=$newtechnologies->setObject($obj);
		if($newtechnologies->add($obj)){
			$error=SUCCESS;
			return $error;
			//redirect("addnewtechnologies_proc.php?id=".$newtechnologies->id."&error=".$error);
		}
		else{
			$error=FAILURE;
		}
...
if(!empty($id)){
	...
	$obj=$newtechnologies->fetchObject;
...
include "addnewtechnologies.php";
?>