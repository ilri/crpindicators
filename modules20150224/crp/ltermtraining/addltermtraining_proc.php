...
//Authorization.
if(!empty($_GET['id'])){
	$auth->roleid="8895";//Edit
}
else{
	$auth->roleid="8893";//Add
}
...
if($obj->action=="Save"){
	...
	$error=$ltermtraining->validate($obj);
	if(!empty($error)){
		$error=$error;
	}
	else{
		//$ltermtraining=$ltermtraining->setObject($obj);
		if($ltermtraining->add($obj)){
			$error=SUCCESS;
			redirect("addltermtraining_proc.php?id=".$ltermtraining->id."&error=".$error);
		}
		else{
			$error=FAILURE;
		}
...
if(!empty($id)){
	...
	$groupby="";
	$orderby="";
	$ltermtraining->retrieve($fields,$join,$where,$having,$groupby,$orderby);
	$obj=$ltermtraining->fetchObject;
