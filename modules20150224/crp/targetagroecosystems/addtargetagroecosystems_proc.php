...
//Authorization.
if(!empty($_GET['id'])){
	$auth->roleid="8927";//Edit
}
else{
	$auth->roleid="8925";//Add
}
...
if($obj->action=="Save"){
	...
	$error=$targetagroecosystems->validate($obj);
	if(!empty($error)){
		$error=$error;
	}
	else{
		//$targetagroecosystems=$targetagroecosystems->setObject($obj);
		if($targetagroecosystems->add($obj)){
			$error=SUCCESS;
			redirect("addtargetagroecosystems_proc.php?id=".$targetagroecosystems->id."&error=".$error);
		}
		else{
			$error=FAILURE;
		}
...
if($obj->action=="Update"){
	...
	if(!empty($error)){
		...
		//$targetagroecosystems=$targetagroecosystems->setObject($obj);
		if($targetagroecosystems->edit($obj)){
			$error=UPDATESUCCESS;
			redirect("addtargetagroecosystems_proc.php?id=".$targetagroecosystems->id."&error=".$error);
		}
		else{
...
if(!empty($id)){
	...
	$obj=$targetagroecosystems->fetchObject;
...
include "addtargetagroecosystems.php";
?>