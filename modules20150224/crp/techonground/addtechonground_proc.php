...
//Authorization.
if(!empty($_GET['id'])){
	$auth->roleid="8939";//Edit
}
else{
	$auth->roleid="8937";//Add
}
...
if($obj->action=="Save"){
	...
	$error=$techonground->validate($obj);
	if(!empty($error)){
		$error=$error;
	}
	else{
		//$techonground=$techonground->setObject($obj);
		if($techonground->add($obj)){
			$error=SUCCESS;
			redirect("addtechonground_proc.php?id=".$techonground->id."&error=".$error);
		}
		else{
			$error=FAILURE;
		}
