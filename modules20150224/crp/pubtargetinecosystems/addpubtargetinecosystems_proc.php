...
//Authorization.
if(!empty($_GET['id'])){
	$auth->roleid="8915";//Edit
}
else{
	$auth->roleid="8913";//Add
}
...
if($obj->action=="Save"){
	...
	$error=$pubtargetinecosystems->validate($obj);
	if(!empty($error)){
		$error=$error;
	}
	else{
		//$pubtargetinecosystems=$pubtargetinecosystems->setObject($obj);
		if($pubtargetinecosystems->add($obj)){
			$error=SUCCESS;
			redirect("addpubtargetinecosystems_proc.php?id=".$pubtargetinecosystems->id."&error=".$error);
		}
		else{
			$error=FAILURE;
		}
...
if($obj->action=="Update"){
	...
	if(!empty($error)){
		...
		//$pubtargetinecosystems=$pubtargetinecosystems->setObject($obj);
		if($pubtargetinecosystems->edit($obj)){
			$error=UPDATESUCCESS;
			redirect("addpubtargetinecosystems_proc.php?id=".$pubtargetinecosystems->id."&error=".$error);
		}
		else{
...
if(!empty($id)){
	...
	$obj=$pubtargetinecosystems->fetchObject;
...
include "addpubtargetinecosystems.php";
?>