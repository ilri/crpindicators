...
//Authorization.
if(!empty($_GET['id'])){
	$auth->roleid="8923";//Edit
}
else{
	$auth->roleid="8921";//Add
}
...
if($obj->action=="Save"){
	...
	$error=$strategicvchainsanalysed->validate($obj);
	if(!empty($error)){
		$error=$error;
	}
	else{
		//$strategicvchainsanalysed=$strategicvchainsanalysed->setObject($obj);
		if($strategicvchainsanalysed->add($obj)){
			$error=SUCCESS;
			redirect("addstrategicvchainsanalysed_proc.php?id=".$strategicvchainsanalysed->id."&error=".$error);
		}
		else{
			$error=FAILURE;
		}
...
if($obj->action=="Update"){
	...
	if(!empty($error)){
		...
		//$strategicvchainsanalysed=$strategicvchainsanalysed->setObject($obj);
		if($strategicvchainsanalysed->edit($obj)){
			$error=UPDATESUCCESS;
			redirect("addstrategicvchainsanalysed_proc.php?id=".$strategicvchainsanalysed->id."&error=".$error);
		}
		else{
...
if(!empty($id)){
	...
	$obj=$strategicvchainsanalysed->fetchObject;
...
include "addstrategicvchainsanalysed.php";
?>